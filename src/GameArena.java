 ...
 *
 * Instances of the Ball and Rectangle classes can be added to an instance of
 * this class to draw and animate simple shapes on the screen.
 *
 * @see Ball
 * @see Rectangle
 *
...
public class GameArena {
	...
	// Collections of primitives. These now relate 1:1 to JavaFX Nodes, since
	// moving from AWT.
	...
	/**
	 * Constructor. Creates an instance of the GameArena class, and displays a
	 * window on the screen upon which shapes can be drawn.
	 *
	 * @param width
	 ...
	 *            The height of the window, in pixels.
	 * @param createWindow
	 *            Determines if a JFrame containing the GameArena should be
	...
	private void frameUpdate() {
		...
		for (Object o : addList) {
			if (o instanceof Ball) {
				Ball b = (Ball) o;
				javafx.scene.shape.Circle c = new javafx.scene.shape.Circle(0,
						0, b.getSize());
		...
		for (Map.Entry<Ball, javafx.scene.shape.Circle> entry : balls
				.entrySet()) {
			Ball b = entry.getKey();
		...
		for (Map.Entry<Rectangle, javafx.scene.shape.Rectangle> entry : rectangles
				.entrySet()) {
	 ...
	 *
	 * @param b
	 *            the ball to add to the GameArena.
	 */
		...
		synchronized (this) {
			if (objectCount > MAXIMUM_OBJECTS) {
				...
				System.out
						.println(" ********************************************************* ");
				System.out
						.println(" ***** Only 100000 Objects Supported per Game Arena! ***** ");
				System.out
						.println(" ********************************************************* ");
				System.out.println("\n");
			...
			// Add this ball to the draw list. Initially, with a null JavaFX
			// entry, which we'll fill in later to avoid cross-thread
			// operations...
			removeList.remove(b);
			addList.add(b);
			objectCount++;
	...
	/**
	 * Remove a Ball from the GameArena. Once a Ball is removed, it will no
	 * longer appear on the window.
	 *
	 * @param b
	 *            the ball to remove from the GameArena.
	 */
	...
	/**
	 * Adds a given rectangle to the GameArena. Once a Rectangle is added, it
	 * will automatically appear on the window.
	 *
	 * @param r
	 *            the rectangle to add to the GameArena.
	 */
	...
	/**
	 * Remove a Rectangle from the GameArena. Once a Rectangle is removed, it
	 * will no longer appear on the window.
	 *
	 * @param r
	 *            the rectangle to remove from the GameArena.
	 */
	...
	/**
	 * Pause for a 1/50 of a second. This method causes your program to delay
	 * for 1/50th of a second. You'll find this useful if you're trying to
